...
export const Footer = () => {
  ...
  return (
    <footer className="text-white footer-bg-class px-4 py-5  mx-auto md:px-8">
      <div className="grid grid-cols-1 md:grid-cols-3">
        <div className="flex-1 mt-5 items-center ml-3">
          <div className="max-w-xs">
            <h1 className=" font-bold">About Us</h1>
            <p className="leading-relaxed mt-2 text-[15px]">
              Founded in 2022, Bharat Hi-Tech Films is a distinguished firm,
              engaged in supplying and trading an extensive range of Cable Wrap
              Material, Thermal Lamination Films, Electrical Motor Insulation,
              Silicone Release Liners, Car Wrapping and Protection Film, Poly
              Films, Plain Aluminium Tape and etc.
            </p>
          </div>
        </div>
        <div className="grid grid-cols-1 mt-5 gap-3 md:grid-cols-2 ml-3">
          {footerNavs.map((item, idx) => (
            <ul className="space-y-2" key={idx}>
              <h4 className="text-gray-50 font-bold">{item.label}</h4>
              {item.items.map((el, idx) => (
                <li key={idx}>
                  <Link
                    to={el.href}
                    className="hover:underline hover:text-red-50"
                  >
                    {el.name}
                  </Link>
                </li>
        ...
        <div className="mt-5 items-center ml-3">
          <h1 className=" font-bold md:text-center">Contact Us</h1>
          <ul className="flex flex-col md:items-start gap-y-8 mt-3">
            {contactMethods.map((item, idx) => (
              <li key={idx} className="flex flex-col md:gap-x-3 md:flex-row">
                <div className="w-8 h-8 mx-auto bg-indigo-50 text-red-600 rounded-full flex items-center justify-center">
                  {item.icon}
                </div>
                <div>
      ...
      <div className=" py-6 border-t items-center justify-between sm:flex">
        <div className="mt-4 sm:mt-0">&copy; 2023 BHT All rights reserved.</div>
